...
const updatePasswordLinkRegExp = `^http:\\/\\/localhost:\\d{4}\\/[a-zA-Z0-9-_]+\\/update-password\\?user=.*$`
...
const forgotPasswordFlow = defineSmokeTest('succeeds creating application', () => {
  ...
  cy.task('getLastLogLineMatching', updatePasswordLinkRegExp).then(temporaryPasswordLink => {
